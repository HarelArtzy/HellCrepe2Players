...
from classes.EnemyState import EnemyState
from classes.LevelState import LevelState
from classes.PlayerInput import PlayerInput
from classes.ProjectileState import ProjectileState
from classes.SimPlayer import SimPlayer
from env_config import ASSETS_DIR, PROJECT_ROOT
...
class DungeonGameServer:
    ...
    async def send_message(self, conn: asyncio.StreamWriter, payload: dict):
        """Serialize and send one pickled payload over an asyncio stream."""
        body = pickle.dumps(payload, protocol=pickle.HIGHEST_PROTOCOL)
        conn.write(f"{len(body)}#".encode("ascii") + body)
        await conn.drain()

    async def read_message(self, reader: asyncio.StreamReader) -> dict:
        """Read one length-prefixed pickled payload from the stream."""
        ...
        while True:
            ...
            for pid, writer in list(self.writers.items()):
                if pid not in self.players:
                    continue
                try:
                    await self.send_message(writer, self.build_snapshot(pid))
                except Exception:
                    logger.exception(
        ...
        try:
            while True:
                try:
                    ...
                    logger.warning(
                        "Received invalid payload from player %s", player_id)
                    continue
