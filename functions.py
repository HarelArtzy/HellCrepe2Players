...
def shoot(projectiles: list, player):
...
async def pause_menu(window, screen, clock):
    while True:
        display_text(
            window, screen,
...
async def end_menu(window, screen, clock, msg):
    while True:
...
async def open_chest(window, screen, clock, level):
    level_str = "lvl" + str(level)
    msgs = MAP_DICT[level_str]["chest_msg"]

    for m in msgs:
        space_was_down = True  # force release first
        while True:
            display_text(window, screen, m, pygame.Color("white"), BASE_W, BASE_H, SCALE)
            clock.tick(30)
...
def make_enemies(level_str):
    from classes import Pancake, Waffle
    enemy_map = {
        "Pancake": Pancake,
        "Waffle": Waffle
    }
